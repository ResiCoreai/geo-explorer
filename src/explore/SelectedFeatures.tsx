...
export function SelectedFeatures() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const lon = pointFeature.geometry.coordinates[0] ?? 0;
      const lat = pointFeature.geometry.coordinates[1] ?? 0;
  ...
  const featureAttributes: Record<string, unknown>[] = useMemo(() => {
  ...
  if (featureAttributes.length === 0) return null;
  ...
  const handleNext = () => {
    setPageIndex((prev) =>
      prev < featureAttributes.length - 1 ? prev + 1 : prev,
    );
  };
      ...
      <Card className="rounded-md shadow-md overflow-hidden text-[14px] leading-[100%] tracking-[0.15px] p-2">
        ...
        <Box className="flex justify-end py-2 px-2">
          <Button
            ...
            className="!normal-case font-normal text-[14px] tracking-[0.15px]"
