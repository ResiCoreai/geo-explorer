...
export function MapLayerSettings() {
    ...
    if (!(selectedLayer && showLayerSettings))
        return null;
    return (_jsxs(_Fragment, { children: [_jsx(Stack, { direction: "column", className: "w-full items-center", children: _jsxs(Stack, { ref: containerRef, direction: "column", className: classNames('bg-white gap-[16px] px-[32px] py-[16px] box-border', floating
                    ...
                    }, onMouseDown: onMouseDown, children: [_jsx(Header, { onOpenStyleSettings: () => setStyleSettingsOpen(true), onClose: () => {
                                dispatch(toggleLayerSettings());
                                setState(State.Attached);
                            } }), selectedLayer.data.dataset_info.timestamps.length > 0 && (_jsx(TimeSelector, {})), isVectorData(selectedLayer) ? (_jsx(Box, { className: "flex-1 min-h-0 cursor-auto", children: _jsx(WFSFeatureTable, { dataset: selectedLayer.data }) })) : null] }) }), _jsx(StyleSettings, { open: styleSettingsOpen, onClose: () => setStyleSettingsOpen(false) })] }));
}
