...
export function StyleSettings({ open, onClose }) {
    ...
    if (!selectedLayer)
        return null;
    return (<Modal open={open} onClose={onClose} disablePortal>
      <Paper className="fixed left-0 right-0 top-0 bottom-0 w-[535px] h-[60%] m-auto rounded-[6px] overflow-scroll no-scrollbar">
        <Stack direction="row" className="sticky top-0 pt-[24px] px-[32px] bg-white z-[1]">
        ...
        <Box className="pb-[24px] px-[32px]">
          {selectedLayer.data.dataset_info.dataset_type === 'vector' &&
            selectedLayer.data.dataset_info.feature_type === 'point' && (<>
                ...
                <NumberInput value={style.radius} onChange={(value) => {
                updateStyleDebounced({
                    radius: value,
                });
            }} min={1} max={20} renderValue={String}/>
              </>)}
          ...
          {selectedLayer.data.dataset_info.dataset_type === 'vector' &&
            (selectedLayer.data.dataset_info.feature_type === 'point' ||
                ...
                <ColorInput value={parseColor(style.fillColor).withChannelValue('alpha', style.fillOpacity)} onChange={(color) => {
                updateStyleDebounced({
          ...
          {selectedLayer.data.dataset_info.dataset_type !== 'raster' && (<>
              ...
              <NumberInput value={style.strokeWidth} onChange={(value) => {
                updateStyleDebounced({
                    strokeWidth: value,
                });
            }} min={0} max={10} renderValue={String}/>
              <Divider className="my-[20px]"/>
              <Typography className="font-bold text-[16px]">
                Stroke Color
              </Typography>
              <ColorInput value={parseColor(style.strokeColor).withChannelValue('alpha', style.strokeOpacity)} onChange={(color) => {
                updateStyleDebounced({
          ...
          <NumberInput value={style.layerOpacity} onChange={(value) => {
            updateStyleDebounced({
                layerOpacity: value,
            });
        }} min={0} max={1} step={0.1} renderValue={(alpha) => alpha * 100 + '%'}/>
