...
export function TimeSelector() {
    ...
    return (<Stack direction="row" className="items-center gap-[12px] cursor-pointer">
      ...
      <Stack direction="row" className="flex-auto min-w-0 overflow-x-scroll no-scrollbar gap-[8px] py-[10px]">
        {yearsAndQuarters.map(({ year, quarters }) => (<Stack direction="column" key={year} className="flex-none w-[80px] gap-[4px]">
            <IconButton className={classNames('h-[18px] leading-[18px] text-center text-[14px] font-semibold rounded-none', year === curYear
                ? 'bg-[#1976D2] text-[white]'
                : 'bg-[#2C343C1A] text-[#2C343C]')}>
              {year}
            </IconButton>
            <Stack direction="row" className="self-stretch justify-between">
              {quarters.map((timestampIdx, quarter) => (<IconButton key={quarter} className={classNames('h-[18px] w-[18px] rounded-none', year === curYear && quarter === curQuarter
                    ? 'bg-[#1976D2] text-[white]'
