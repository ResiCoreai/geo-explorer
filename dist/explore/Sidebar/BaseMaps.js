...
export const BaseMaps = () => {
    ...
    return (_jsxs(Box, { className: "flex-none h-[80px] items-center bg-[#F4F5F7] p-[12px]", children: [_jsx(Box, { className: "flex-grow-1", children: _jsx(Typography, { variant: "caption", color: "gray", children: "Basemap" }) }), _jsx(Select, { value: activeBaseMap || '', onChange: handleChange, variant: "standard", disableUnderline: true, className: "w-full bg-[#F4F5F7]", classes: {
                    select: 'flex items-center justify-between', // Equivalent of `.MuiSelect-select`
                }, children: baseMaps.map((basemap) => (_jsxs(MenuItem, { value: basemap.layer_id, className: "flex items-center justify-between w-full", children: [basemap.display_name, _jsx(ListItemIcon, { children: _jsx(Box, { component: "img", src: basemap.thumbnail_url, alt: basemap.display_name, className: "w-[50px] h-[40px] rounded-[4px] mr-[1]" }) })] }, basemap.layer_id))) })] }));
};
