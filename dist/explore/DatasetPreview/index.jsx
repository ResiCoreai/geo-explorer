...
export function DatasetPreview() {
    ...
    if (!dataset)
        return null;
    return (<Modal open={!!dataset} onClose={onClose}>
      <Paper className="fixed left-0 right-0 top-0 bottom-0 m-auto rounded-[6px] py-[24px] px-[32px] overflow-scroll no-scrollbar w-[60%] h-fit">
        <Stack direction="row" className="items-center">
          <Typography className="font-bold text-[20px]">
            {dataset === null || dataset === void 0 ? void 0 : dataset.display_name}
          </Typography>
          <Box className="flex-1"/>
          {dataset &&
            (addedToLayers ? (<Button variant="contained" disableElevation className="bg-[#2C343C]" startIcon={<RemoveOutlined />} onClick={() => {
                    dispatch(removeLayer({ layer_id: dataset.layer_id }));
                }}>
                Remove
              </Button>) : (<Button variant="contained" disableElevation startIcon={<AddOutlined />} onClick={() => {
