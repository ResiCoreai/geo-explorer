...
export const initialize = createAsyncThunk("explore/initialize", getInitialSettings);
    ...
    reducers: {
        selectDataset(state, action) {
            if (action.payload.layer_id == null) {
                ...
                const dataset = state.dataInventory.find((l) => l.layer_id === action.payload.layer_id) ||
                    state.climateInventory.find((l) => l.layer_id === action.payload.layer_id);
                if (dataset) {
...
export const { selectDataset, selectMapLayer, toggleLayerSettings, setShowLayerSettings, selectBaseMap, addLayer, removeLayer, reorderStart, reorderEnd, setCurrentIndex, toggleVisibility, togglePlaying, setTimestampIdx, setSelectedFeatures, setLayerStyle, resetLayerStyle, } = exploreSlice.actions;
